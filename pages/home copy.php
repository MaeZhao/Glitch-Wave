...
if (isset($_GET['q'])) {
  ...
  if ($search_terms) {
    // sql clauses
    $sql_select_query = $sql_select_query . " WHERE ((title LIKE '%' || :search || '%') OR (album LIKE '%' || :search || '%') OR (artist LIKE '%' || :search || '%')  OR (:file_name LIKE '%' || :search || '%'))";
...
// ORDER BY SQL Calls
$table_param = array('title', 'artist', 'rating', 'album', 'file_name');
  ...
  <main>
    <div class="center">
      ...
      <table class="center">
        <tr class="table_head">
          ...
          <th class="file_name"><a href="<?php echo $sort_url . '&amp;sort=file_name&amp;order=' . $order_next_url['file_name']; ?>"> file_name <?php if ($sort_css_buttons['file_name_desc']) {
                                                                                                                                ?>&#8638;<?php
                                                                                                                                        } elseif ($sort_css_buttons['file_name_asc']) { ?>&#8643;<?php
                                                                                                                                                                                      } elseif (!($sort_css_buttons['file_name_desc'] && $sort_css_buttons['file_name_asc'])) { ?>&#8638;&#8643;<?php } ?></a></th>
        </tr>
        <?php
        ...
        foreach ($records as $record) { ?>
          <tr>
            ...
            <td class="rating_item"><?php make_stars((int)$record["rating"]);
                                    ?></td>
            <td class="file_name_item"><?php
                                    if (!empty($record["file_name"])) {
                                    ?><a href="<?php echo htmlspecialchars($record["file_name"]); ?>"><?php echo htmlspecialchars($record["file_name"]); ?></a> <?php } ?></td>
          </tr>
        <?php } ?>
      </table>
    </div>
    ...
    <div class="center" id="song_form">
      ...
      <?php if ($show_form && $is_admin) { ?>
        <h2>Add Song:</h2>
        <form id="add_song" action="#song_form" method="post" novalidate>
          <div class="input">
